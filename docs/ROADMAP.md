...
直近に完了したマイルストーンは、棋力向上段階2（plans/strength-stage2.md、2026年9月4日）である。
手数に基づく残り手数見積りの予算式、次反復の完了予測による反復継続の判断、および`position`再生の差分適用を採用し、停止理由の記録を段階開始版として残した。
予算式と反復継続の判断はそれぞれSTCとLTCが`H1`であり、最終構成と段階開始版の段階ゲートGSPRTは測定費用を理由とする利用者の決定により項目ごとのLTCの連鎖で代えた。
段階開始版との固定200ペアはSTCで+113.9 Elo、HaChu戦は段階1完了時の+79.5 Eloから+149.3 Eloへ上昇し、HaChu側の不正着手1件を反則負けとして算入した。
これらの測定は段階間の進捗指標であるため、変更の採否には用いない。

...
| 棋力向上段階2 | [plans/strength-stage2.md](plans/strength-stage2.md) | 起案 | ― |

## 現在地

直近に完了したマイルストーンは、棋力向上段階1（plans/strength-stage1.md、2026年9月2日）である。
捕獲専用生成、評価値の差分更新、および段階的な手選択を採用し、手配列の再利用と反復回数表はbenchの採用基準を満たさなかったため除いた。
最終構成は段階開始版とのLTCで`H1`かつ異常0件となり、固定200ペアの自己対局とHaChu戦を進捗指標として記録した。
HaChu戦は前回の−111 Eloから+79.5 Eloへ上昇し、HaChu側の異常4件を反則負けとして算入した。
この測定は段階間の進捗指標であるため、変更の採否には用いない。

進行中のマイルストーンは、上位計画の棋力向上の段階計画（plans/strength-stages.md）である。
10段階のうち段階1が完了し、段階2（plans/strength-stage2.md）は2026年9月2日に起案して着手前である。
段階1の保存記録から時計を再構成した結果、探索は序盤で主時間を使い切り、100手目以降は7〜9割の手がhard limitで打ち切られていることが分かった。
段階2は予算式の再設計、反復継続の判断、および停止理由の記録を扱い、時間管理の適応的な延長と係数の再調整は段階6へ移した。
次の一手は、段階2のフェーズ1の停止理由の記録に着手することである。
...
次期候補は、段階2の完了後の棋力向上段階3の静止探索の質である。

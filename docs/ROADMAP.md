...
[訓練集合23,879,611局面での再学習](measurements/strength-stage7-gen2-training.md)は10エポックを計算し、第8エポックの検証損失0.478427486が最小となったが、量子化時に重みが16ビット符号付き整数の範囲を超えて停止した。
同条件の再実行で重みを採取し、範囲超過が相手獅子の序中盤重みの鏡映1対に限られることを確認した。
独立した整数格納と鏡映展開の検査でも整合し、学習規則が重みの範囲を制約しないため最良状態を保存できないことが分かった。
利用者が追加を決めた保存範囲への射影を学習器へ実装し、PST関連の全45テストと誤実装の検出検査を完了した。
候補重みは未出力であり、次の一手は新しい実行を準備し、保存済みの全11入力と同じ初期重みから再学習することである。

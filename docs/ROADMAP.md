...
同じ候補のPSTとの短時間比較を開始した。PSTより強いことと採用はまだ確定していない。
現行の評価は、短時間と長時間の採用条件を満たした2端点PSTである。

進行中のマイルストーンは3件である。
補正1/4のFMの採否（plans/fm-quarter-adoption.md）では、採用PSTとの短時間比較を実行中である。
...
FM補正を1/4に縮めた候補はPSTとの短時間比較を実行中であり、通過時は長時間測定へ進む。

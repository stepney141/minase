...
| 合法手生成と利き計算の高速化 | [plans/movegen-speedup.md](plans/movegen-speedup.md) | 進行中 | ― |
...
進行中のマイルストーンは、上位計画の[棋力向上の段階計画](plans/strength-stages.md)と、[合法手生成と利き計算の高速化](plans/movegen-speedup.md)である。
高速化は計測基盤と単位A（利き逆引きの片側化、方向主導化、特殊利きの走査限定）が完了し、NPSは基準比1.319倍である。
次に単位B（静止探索の捕獲対象の事前除外と段階生成）を検証点ごとに確定する。
棋力向上の段階計画は10段階のうち段階7までが完了し、次の段階は採用構成と世代2のデータを起点に進められる。

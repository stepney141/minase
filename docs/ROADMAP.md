...
プロトコル層の完了後、一時状態表現の統合から着手して状態を進行中へ変更する。
順方向の探索部と評価関数は直前局面生成器の完了を前提としない。
...
リプレイ照合は10局・全2,462手で一致し、その過程でUSI解析の3升連結正規化の漏れ1件を修正し、lishogiの裸玉裁定と第22条の駒枯れの規則差1件を記録した。
テストは237件全緑であり、経緯は設計書 plans/protocol-layer.md にある。
次の作業はプロトコル層のフェーズ5（CECP実装）である。

...
採用済みの鏡映共有モデルで[世代2の生成と検証を完了した](measurements/strength-stage7-gen2-generation.md)。
...
量子化時の範囲超過で停止した[世代2の再学習](measurements/strength-stage7-gen2-training.md)に対し、利用者が追加を決めた保存範囲への射影を実装して検証した。
新しい実行ディレクトリを準備し、全11入力、初期重み、探索用駒価値47個、および学習条件の一致を確認した。
既存の識別性診断を再利用できることを独立監査でも確認し、訓練集合23,879,611局面でGPU再学習を正常に終えて候補を書き出した。
候補診断で駒の除去差分の符号反転を1件検出し、独立した整数計算、浮動小数点重み、およびRust評価でも再現した。
診断基準未達で候補を除外し、STCとLTCは実施せず、採用済みの鏡映共有モデルを維持した。
フェーズ5では、採用中の重みと世代0〜2の訓練局面の平均補間係数から[探索用駒価値を再導出した](measurements/strength-stage7-values-diag.md)。
到達可能な37状態の最大相対差は2.830189%で基準20%を超えなかったため、固定値を維持した。
固定200ペアの2測定に使うシードの監査を完了し、段階開始版との測定を実行中である。
完了後にHaChu戦を実行し、両結果を進捗指標として記録する。

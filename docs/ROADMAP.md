...
第2版では、静止探索での置換表の照会と記録（ノード固定GSPRTでH1、固定深さ5のノード数−53%）、最深ワーカーの結果採用と周期付き深さスキップ（固定深さbenchで`Threads=2`が深さ5で−26.5%、深さ6で−17.9%）を採用し、`time=30000+300,byoyomi=500`の時間制御GSPRTで`Threads=2`対1が500ペア上限でLLR 2.615の判定保留ながら固定局数Elo +62［+41, +83］となり、信頼区間が0を大きく上回ることを根拠に採用と判断した（記録と規定からの逸脱の根拠は plans/lazy-smp.md の実施状況）。

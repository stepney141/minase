...
次は同じ候補で長時間測定へ進む。長時間測定は未開始であり、採用はまだ確定していない。
現行の評価は、短時間と長時間の採用条件を満たした2端点PSTである。

進行中のマイルストーンは3件である。
補正1/4のFMの採否（plans/fm-quarter-adoption.md）では、採用PSTとの短時間比較を通過し、長時間測定の開始を次の作業とする。
...
FM補正を1/4に縮めた候補はPSTとの短時間比較を通過したため、長時間測定へ進む。

...
直近に完了したマイルストーンは[合法手生成と利き計算の高速化](plans/movegen-speedup.md)であり、利き逆引きの片側化、静止探索の段階的な捕獲生成、SEEの早期終了、およびPGOでbenchのNPSを基準比1.794倍にし、最終のSPRTはSTCとLTCの両方で`H1`だった。
releaseビルドはPGOのプロファイルを照合するので、ソースを変えたら`scripts/pgo_profile.py`で再生成する。

...
検証損失は初期状態を含む各エポックで世代別に記録し、`<出力名>.training.json`へ保存する。
追加の`game_half`は既存の検証対局のうち`hash64(seed, game) % 40 == 0`となる側の局面平均であり、訓練集合や最良エポックの選択対象は変えない。
王駒が2枚の側を含む局面とそれ以外についても件数と損失を記録し、空の区分は`null`とする。

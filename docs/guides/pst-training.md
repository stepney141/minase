...
| `train.rescore` | 付け直しファイルを`run.data`、生成シードの順に列挙する。付け直しなしは`"-"`とし、項目を省略した場合は全ファイルを付け直しなしとする。 |
| `train.lambda_override` | 自己対局の教師混合比を0以上1以下の有限値で上書きする。省略時は来歴のλを使い、明示的な0は対局結果だけを使う。実戦棋譜のλは変えない。 |
| `train.lookahead` | 先読みを使う場合は`{ gamma = 0.9, plies = 40 }`を指定する。省略時は元の探索値を使う。γは0より大きく1未満、手数上限は整数1〜4,096とする。 |
...
`game_half`は従来どおり`hash64(seed, game) % 40 == 0`となる検証局面の平均である。
追加の`human_game_mean`は対局結果が実戦棋譜に由来する局面について、同じ棋譜IDの局面損失を平均した後、IDごとの平均を等しい重みで平均する。
...
各MNSDには、パス末尾に`.provenance.json`を付けた来歴ファイルが必須である。
...
本段階では自己対局を0.75、実戦棋譜の対局結果だけを使う分類を0とする。
...
計算式と記録が欠けた場合の正規化は[先読み教師値の設計書](../plans/lookahead-teacher.md#先読み値の計算)に従う。
...
この診断は学習器と同じ訓練分割を使い、γごとの先読み値と探索値の差、駒数帯ごとの差、差が−300センチポーン以下の割合、窓の記録数と退避率を出す。
分布には平均、母標準偏差、および線形補間による5、25、50、75、95パーセンタイルを記録する。
最初の記録までの手数は、記録が1件以上ある窓だけで集計する。
γの一覧には0.9を含め、教師K、露出群と対照群の教師勝率差、および118列との残差相関はγ=0.9で比較する。
...
乱数開始の自己対局では`games`を`null`とし、検証の所属は従来の`hash64(seed, game) % 20 == 0`で決める。
その他の由来では`games`を`{"game": 1, "id": "棋譜ID"}`形式の配列とし、実戦開始では各要素に開始手数`ply`も記す。
全記録の対局番号が対応表に必要であり、対応しない番号があれば停止する。
...
生成、学習、診断、および再開時に来歴と付け直しの検査和も照合し、変更があれば停止する。
準備後に生成するMNSDの来歴は、生成完了時の記録へ検査和を固定する。
...
λ=0の分類ではKを推定せず、JSONでは`null`とし、教師値には対局結果だけを使う。
...
分類ごとの尺度、訓練と検証の局面数、1エポックと全体の更新回数は `training/inputs.json` に記録する。
...
追加係数は来歴ファイルの教師混合率`lambda`と区別し、式の定義は[評価関数の学習](../plans/evaluation.md#教師値と損失)に従う。
...
これらの損失に加え、全体と分類別の検証損失、最良エポック、特徴の観測回数を読み、更新回数の不足や過学習がないか点検する。
...
`bands` は、補間係数を5等分した局面帯と教師の分類ごとに、訓練と検証の局面数、全検証局面の検証損失、および最大 `diagnose.sample_size` 局面の標本について教師探索値との平均絶対誤差（出力Kと教師Kの比で換算した値と生の値）と相関を基準と候補で比較する。
...
これらは教師の混合比に依存しない診断値であり、採否は自己対局測定で決める。
...
| `generated-*.bin`、`generated-*.bin.provenance.json`、`generated-*.json` | 生成データと検査済みの検査和を保持する。 |

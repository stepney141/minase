...
- `decision: H0`（elo=10の改善があるとは言えない）は、候補を採用しない。
...
固定200ペアのEloは95%信頼区間の幅が約65 Eloあり、個々の変更の効果量を測る精度はない。
...
### 先読みを対局条件に含める測定（`--ponder`）

`--ponder`は、USIの先読み（ponder）を対局進行に含める対局条件である（設計は [plans/ponder.md](../plans/ponder.md)）。
ハーネスは、`bestmove X ponder Y`の形で予想手を返したエンジンにだけ、相手の手番中に`position ... X Y`と`go ponder`を送る。
相手の着手が予想手と一致すれば`ponderhit`を送り、一致しなければ`stop`を送って`bestmove`を1行読み捨ててから通常の`go`を送る。
この手順は候補と基準に同じく適用され、ハーネスはどちらが先読みに対応するかを知らない。
したがって`--ponder`は時間制御と同じ対局条件であり、機能比較のスイッチではない。

`--ponder`は両エンジンが時間制御（`time=`）の場合にだけ指定でき、`depth`または`nodes`の制限、および`cecp:`のエンジンとの併用はエラーになる。
外れた先読みを止める時間は、先読みをした側の消費時間に含まれる。
予想手が審判層の合法手でない場合、およびその手で終局する場合は先読みをさせず、反則負けにもしない。
最終サマリの`ponder candidate:`行と`ponder baseline:`行は、予想手の出力数、不合法な予想手の数、`go ponder`の送信数、的中数、および着手数を、保存記録の再生から数えて出力する。
先読みの実装を変更する候補の採否では、候補の`illegal_predictions`が0であることも採用の条件にし、両者のCPU時間、`go ponder`の送信数、および的中数を測定記録へ残す。

`--ponder`の測定が示すのは「同じ時計の下で、相手の手番中の計算も使える実装の強さ」であり、同じCPU時間あたりの効率ではない。
段階ゲートの標準コマンドは`--ponder`を含まない。
`--ponder`は、先読みの挙動を変える変更の採否と、先読みを使う運用条件での強さの確認に使う。
...
`--ponder`の測定では先読みの探索時間もプロセスのCPU時間に含まれるので、CPU時間に依存する校正用の指標は`--ponder`なしの測定と比較できない。
...
ハーネスは、USIエンジンの起動時に`setoption name ResignValue value 99999`を候補と基準の両方へ無条件に送り、エンジンの投了（[USI投了](../plans/usi-resignation.md)）を無効にして対局を規則上の終局まで指させる。
投了が測定に入ると、相手が先に投了した局面からの勝ち切りの差を測れず、保存記録も完走した対局でなくなるからである。
このオプションを持たないコミットは、USIの慣例どおり未知のオプション名を無視する。
...
通常実行の外部参照値と境界判定だけでは、参照点以外での累積的な統計誤りを検出できないため、変更時にはモンテカルロ検証も完了させる。

...
- [予算の機構を変えたら総思考時間と使い残しを現行と比べる](compare-total-time-usage-before-sprt.md) — 予算の機構を変える候補は、採否測定前の煙試験で総思考時間が現行の90〜110%に収まり完了深さがどの帯でも下回らないことを確かめ、中断方式では中断が着手を変えた割合も数える。
- [計測用の共有カウンタはマルチスレッドの速度を歪める](shared-probe-counter-distorts-threads.md) — 計測用のカウンタはワーカーごとの局所変数に置いて探索後に合算し、速度は計測コードなしのバイナリで測り直して、計測コードつきの実行からは回数の比だけを読む。
- [探索値と静的評価の差を学習するときは戦術上の利得を除外する](correction-history-exclude-tactical-gains.md) — 最善手が捕獲手のノードと王駒に利きが届くノードを更新から除外し、候補は補正を使う場面と同じ母集団の予測誤差で比べる。

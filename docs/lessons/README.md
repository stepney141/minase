...
- [着手差は配置変化と手番交代を分けて測る](separate-perspective-change-in-eval-deltas.md) — 評価の着手差では固定視点の配置変化と手番成分を分け、絶対量と同一親局面の候補間変動を別々に測る。
- [教師探索の順位は予算を変えて検証する](check-teacher-ranking-stability.md) — 提案手と候補集合を固定して教師予算を変え、正負反転・零との変化・最上位手集合の変化を区別し、不安定な教師から敗因を断定しない。

# 合法手生成高速化の採用版と基準の最終SPRT（STC）

## 目的

合法手生成と利き計算の高速化の採用版（単位AからDまでを含み、PGOを適用したコミット`5a67d41`）が、基準コミット`0e3dfa6`より短時間条件で有意に強いかをGSPRTで判定し、`H1`ならLTCへ進める。

## コマンドライン

```console
cargo run --release --bin match_runner -- \
  --run-dir data/matches/movegen-speedup-final-stc --seed 20260915 \
...
測定機はIntel Core Ultra 7 265KF（物理20コア、論理20コア、実メモリ32 GB）であり、候補と基準はいずれも`Threads=1`、`USI_Hash`は各256 MB、同時対局数は16である。手数上限は4,096、応答タイムアウトは120秒で、2026年9月15日に実施した。

## 結果

| 項目 | 値 |
|---|---|
| ペア数 | 165（有効158、手数上限到達による破棄7） |
| ペンタノミアル度数 | [13, 3, 68, 10, 64] |
| LLR | 2.9589 |
| 判定 | `H1` |
| 異常件数 | illegal_moves=0 crashes=0 timeouts=0 |
| `time_forfeits` | 0 |
| `rejected_moves` | 0 |
| 経過時間 | 1,485.9秒（`summary.json`の`active_wall_time_ns = 1485890420711`） |

## 結論

候補は短時間条件で`H1`（有意に強い）であり異常0件なので、LTCへ進める。

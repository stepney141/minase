...
基本シード600000、700000、800000の生成と`inspect`は、いずれも終了コード0で完了した。
現在は基本シード900000を生成中であり、全5ファイルのうち3ファイルの検証を終え、合計8,469,843局面を保存した。
完了したシードの結果を次に示す。
...
| 終局理由 | 基本シード600000の局数 | 基本シード700000の局数 | 基本シード800000の局数 |
|---|---:|---:|---:|
| 王駒の捕獲による勝ち | 21 | 23 | 27 |
| 詰みによる勝ち | 6,848 | 6,804 | 6,798 |
| 反復規則による勝ち | 316 | 329 | 347 |
| 駒の枯渇による勝ち | 9 | 13 | 9 |
| 反復規則による引き分け | 157 | 179 | 182 |
| 駒の枯渇による引き分け | 3 | 0 | 1 |
...
3ファイルとも全レコードの独立集計で`inspect`の集計値が一致した。
監査結果は実行ディレクトリの`seed-600000-audit.json`、`seed-700000-audit.json`、および`seed-800000-audit.json`に保存した。
...
3ファイルの生成と検証を完了し、残る2ファイルを生成している。

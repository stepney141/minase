...
基本シード600000、700000、800000、900000の生成と`inspect`は、いずれも終了コード0で完了した。
現在は基本シード1000000を生成中であり、全5ファイルのうち4ファイルの検証を終え、合計11,254,972局面を保存した。
完了したシードの結果を次に示す。
...
基本シード900000では先手勝ち17局と後手勝ち8局だった。
個別の局番号や除外理由は、いずれも保存資料からは特定できない。
終局理由の内訳は次のとおりである。

| 終局理由 | 基本シード600000の局数 | 基本シード700000の局数 | 基本シード800000の局数 | 基本シード900000の局数 |
|---|---:|---:|---:|---:|
| 王駒の捕獲による勝ち | 21 | 23 | 27 | 24 |
| 詰みによる勝ち | 6,848 | 6,804 | 6,798 | 6,821 |
| 反復規則による勝ち | 316 | 329 | 347 | 331 |
| 駒の枯渇による勝ち | 9 | 13 | 9 | 8 |
| 反復規則による引き分け | 157 | 179 | 182 | 161 |
| 駒の枯渇による引き分け | 3 | 0 | 1 | 0 |
...
4ファイルとも全レコードの独立集計で`inspect`の集計値が一致した。
監査結果は実行ディレクトリの`seed-600000-audit.json`、`seed-700000-audit.json`、`seed-800000-audit.json`、および`seed-900000-audit.json`に保存した。
...
4ファイルの生成と検証を完了し、残る1ファイルを生成している。

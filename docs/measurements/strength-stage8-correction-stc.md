...
[棋力向上段階8](../plans/strength-stage8.md)のフェーズ5として、探索値と静的評価の差を駒種別の枚数の鍵ごとに学習し、futility pruningの判定に使う静的評価を補正する構成（correction history）（コミット`a000948`）が、SEEによる捕獲手の枝刈りの採用構成（コミット`6689f27`、探索は`c1c7227`と同一）より短時間条件で有意に強いかをGSPRTで判定し、通過ならLTCへ進める。

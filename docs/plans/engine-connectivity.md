...
Lishogi-Bot経由でテスト用Botアカウントの非レート対局を1局完走し、対局ID、規則、全着手の受理、終局statusを記録する。
Bot APIが中将棋のBot対局と非レート対局を受理することは、フェーズ3の着手前に現行仕様で確認する。
...
- Lishogi-Bot経由の非レート対局が1局完走し、対局IDと終局statusが記録されている。

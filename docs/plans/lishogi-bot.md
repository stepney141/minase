...
- ponder。導入と有効化は[USI先読み](ponder.md)が扱う。minaseは先読みを実装済みであり、Lishogi-Botの設定は同書のフェーズ5が完了するまで無効のままにする。
...
| ponder | [USI先読み](ponder.md)のフェーズ5が完了するまで無効にする。minaseはponder（相手の手番中に先読みする機能）を実装しており、`bestmove`に予想手を付け、`go ponder`と`ponderhit`に対応する。`USI_Ponder`は宣言しない。Lishogi-Botは設定が無効なら予想手を使わず、`go ponder`を送らない。固定しているフォークには、秒読みの消化中に`go ponder`の残り時間が過大になる不具合が残るので、その修正と配備環境での通信の確認を終えてから有効へ切り替える。 |

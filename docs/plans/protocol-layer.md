...
HaChu互換規則セットの検証は不成立であり、フェーズ5の規定どおりプリセット名の導入は見送った（プリセットの導入自体は完了条件に含まれない。詳細と再検討条件は [protocols/hachu.md](../research/protocols/hachu.md) 第11章）。
...
実装に先立ち、既存の中将棋関連ソフトウェアのプロトコル仕様を調査し、結果を`docs/research/protocols/`配下に永続的な参照資料として残す。中将棋のプロトコル仕様は標準将棋やチェスと異なり一次資料が分散しているため、RULES.mdと同様に典拠と参照日を明記した調査文書を設計の正とする。
...
- 既存ソフトウェアの仕様調査と`docs/research/protocols/`の整備。
...
| 調査成果物 | `docs/research/protocols/`配下に調査対象別のファイルと索引を置き、参照日付き典拠を明記する。 |
...
本章は、フェーズ1の調査3文書（docs/research/protocols/）、設計レビュー、独立監査（docs/audits/protocols-audit-2026-08-10.md）の指摘を踏まえた確定設計である。2026年8月10日に確定した。
...
次の3対象を並列に調査し、`docs/research/protocols/`配下へ対象別のファイルとして執筆する。各ファイルには、RULES.mdと同じ様式で参照日付きの典拠を明記する。
...
索引として`docs/research/protocols/README.md`を置き、各ファイルの範囲と本設計書への対応を示す。usi-lishogi.mdには、random-playフェーズ2が参照する規範であることを明記する。
...
フェーズ5では、HaChu互換の規則セットの検証も行う。docs/research/protocols/hachu.mdの調査時点で、HaChuの既定の成り規則（敵陣入域時のみ）に対応する規則コードは存在せず、反復のR2との厳密一致と先獅子の実対局挙動も未確認である。CECP経由のHaChuとの実対局照合（lishogiにおけるリプレイ照合に相当する検証）で対応を確定し、成立した組合せに限りプリセット名を与える。必要な場合は新Pコードの規則書改版を先行させる。検証が成立しない場合、HaChuプリセットは導入しない。

HaChu互換検証は不成立であり、プリセットは導入していない。検証に用いたハーネスは`scripts/hachu_selfplay.py`（CECP経由の自己対局生成）と`scripts/hachu_replay.py`（minaseによるリプレイ照合）である。検証対象のHaChu 0.23（コミット649ef114）は`memory`未受信時の`go`で異常終了するほか、正常終局した全局で基本的な移動規則に反する手（自駒がある升への移動、走り駒の経路上の自駒の無視、手番でない側の駒の移動）を自発出力し、規則セットR2+E1とL1+R2+E1のどちらでも全手合法の局は0局であったため、先獅子・反復・成りの規則差の照合には到達していない。欠陥の詳細と再検討条件は [protocols/hachu.md](../research/protocols/hachu.md) 第11章にあり、生成棋譜は壊れた実装の産物であるため同梱しない。
...
- `docs/research/protocols/`に3対象のファイルと索引が典拠付きで存在する。
...
フェーズ1で作成する`docs/research/protocols/`配下の各文書が、プロトコル仕様の典拠一覧を保持する。

...
- lishogi棋譜のリプレイ照合が、全対局の各手の合法性と終局裁定で一致する。lishogiの裸玉裁定（bareKing）は第22条の駒枯れと条件が異なる記録済みの規則差であるため、該当棋譜の期待値は「最終手適用後もminaseは対局を継続する」とし、この対応を含めて一致を判定する。
...
- lishogi棋譜のリプレイ照合が導入され、記録済みの規則差（裸玉裁定）の期待値対応を含めて全対局で一致する。
- 本節の検証コマンドがすべて成功する。

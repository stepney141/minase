...
完了。
2026年9月26日に起案、着手、および完了した。
探索部、学習PST、および学習データの形式を本書の配置へ移し、深さ6のbenchの出力と全試験が再編前と一致することを確かめてmasterへ統合した。
NPSは再編後の中央値が基点の3回の計測の揺れの範囲を上側へ約0.07%外れたが、benchのバイナリに含まれるminaseの関数の集合と各関数の機械語のバイト数（`nm --size-sort`の値）が基点と一致し、インライン化は変わっていない。

...
起案。
2026年9月26日に起案し、同日に利用者の決定で、αβ探索を`alphabeta/`へ置くこと、進行中の[探索局面を用いた学習](search-aware-evaluation.md)の統合を待たずにmasterで実施すること、および学習データの形式を`src/eval/`の外へ移すことを確定した。
codexと可読性のレビューを反映済みであり、次の一手は基点のbenchを記録してcodexへフェーズ1の実装を委任することである。
...
1つ目は、関数の本体にある完全修飾パス（`src/bin/selfplay_gen.rs`の`minase::eval::training_data::FORMAT_VERSION`など）である。
...
`alphabeta/`の層は方式が1つしかない現状でも「αβ探索に固有の処理」という境界を示し、境界の型がαβ探索の内部に依存していないことを配置で確かめられる。

時間管理を`alphabeta/`に置くのは、予算式が係数表`params`の値を読み、反復深化の開始判定と先読みの的中処理がαβ探索の反復に結びついているからである。
...
旧パスの再公開を置かないのは、利用者がこのリポジトリ内の実行ファイルと試験だけであり、書き換えで足りるからである。

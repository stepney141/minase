...
| RS | docs/plans/usi-resignation.md（設計判断、USI層の契約、検証） |
...
- 前提: 対局中で、RSの投了条件を満たさない。投了する場合の応答はD6-USI-51〜D6-USI-58が定める。
...
- 境界・不正: なし。
- 性質: `go`は`Engine.game`に対して読み取り専用である。`bestmove resign`の場合も同じ契約を適用する（RS、D6-USI-59）。
...
- 前提: 対局中で、RSの投了条件を満たさない。採用した最終結果の主要変化が2手以上あり、予想手がD6-USI-40の検査を通る。投了時には予想手を付けない（D6-USI-58）。
- 操作と期待観測: この前提を満たす探索は`bestmove X ponder Y`を1行返す。Yは、直前の最終`info`行の`pv`の2手目と一致する。Xを着手列へ加えた`position`の後の`moves`応答にYが含まれる（合法性の外形検証）。`go ponder`で始めた探索が返す`bestmove`にも同じ規則で予想手が付く。
- 境界・不正: 予想手の有無は`USI_Ponder`の受信に依存しない。`Threads`が2以上の探索は採用結果が非決定的なので、Yの値を固定せず、最終`info`の`pv`との一致を検証する。共有置換表による打ち切りで主要変化が1手になる場合は、完了深さが2以上でもD6-USI-40に従って予想手を付けない。
...
- 境界・不正: 外れた先読みの`bestmove`はGUIが捨てる値であり、通常着手を返す場合は合法手であること（D7のINV-1）が保たれる。RSの投了条件を満たす場合は`bestmove resign`を返す（D6-USI-54、D6-USI-55）。
...
### D6-USI-47 ResignValueオプションの宣言
- 典拠: RS「USI層の契約」。
- 前提: 起動直後。
- 操作と期待観測: `usi`への応答に`option name ResignValue type spin default 20000 min 1 max 99999`がちょうど1行あり、最後は`usiok`となる。
- 境界・不正: CPU数や局面によって宣言を変えない。
- 性質: 既定値と範囲の宣言が決定的である。

### D6-USI-48 ResignValueの受理、拒否と設定維持
- 典拠: RS「USI層の契約」。エラー本文は同節で固定する。
- 前提: 待機中で、直前に有効な閾値を受理している。
- 操作と期待観測: 1〜99999のASCII数字列を無応答で受理する。値の欠落は`info string error: missing ResignValue value`、非整数と範囲外は`info string error: invalid ResignValue value`を各1行返す。後続の探索は直前に受理した閾値で判定する。
- 境界・不正: 1、99999、先頭ゼロを受理し、空値、0、100000、負数、符号つき、小数、指数表記、非数字、桁あふれを拒否する。
- 性質: 拒否は設定を更新しない。

### D6-USI-49 探索中のResignValue変更
- 典拠: RS「USI層の契約」、D6-USI-22。
- 前提: 閾値99999で探索中、または完了結果を保留中。
- 操作と期待観測: `setoption name ResignValue value 1`を待機列に載せる。`stop`で解放する現在の結果は旧閾値で通常の着手を返し、次の探索は新閾値で投了する。
- 境界・不正: 探索の終了前に新閾値を適用しない。
- 性質: 設定の反映境界は現在のbestmove送出後である。

### D6-USI-50 深さ0の採用結果は投了しない
- 典拠: RS設計判断「判定の入力」。
- 前提: 負の静的評価を持つ局面に閾値1を設定する。
- 操作と期待観測: `go nodes 1`で深さ1完了前に止めると、合法な`bestmove`を1行返す。
- 境界・不正: 静的評価が閾値以下でも投了しない。
- 性質: 探索の裏付けがない結果で対局を放棄しない。

### D6-USI-51 既定値で負の詰み帯から投了する
- 典拠: RS設計判断「既定値」「閾値の与え方」。
- 前提: 相手の次の着手で自分の最後の王駒を必ず取られる局面を`position sfen`で与える。
- 操作と期待観測: 深さ1〜2の完了した探索で負の`score mate`を報告し、既定値20000により`bestmove resign`を1行返す。
- 境界・不正: 手番側の視点で判定し、正の詰み帯では投了しない。
- 性質: 詰み帯にも整数の閾値比較を適用する。

### D6-USI-52 通常評価値の閾値比較は等号を含む
- 典拠: RS設計判断「判定の入力」「閾値の与え方」。
- 前提: 深さ1以上の採用結果が負の通常評価値である。
- 操作と期待観測: 閾値1で投了する。同じ局面の採用結果を閾値無効の探索の`info score cp`から読み、評価値の絶対値と等しい閾値なら投了し、1大きい閾値なら通常の着手を返す。
- 境界・不正: 0と正の評価値では投了しない。
- 性質: 閾値を増やすと投了対象が増えることはない。

### D6-USI-53 詰み帯での閾値境界
- 典拠: RS設計判断「閾値の与え方」。
- 前提: 負の詰み帯を返す局面で深さ1以上が完了する。
- 操作と期待観測: 閾値29744では投了し、30000と99999では通常の合法手を返す。
- 境界・不正: 採用結果−29744と−29998も通常の整数比較で判定し、29745〜29998では詰みまでの距離に応じて結果が変わる。次の相手の着手で最後の王駒を取られる局面の採用結果は−29998（`mate -2`）である。
- 性質: 無効化のための特殊分岐を設けない。

### D6-USI-54 実行中のstopによる投了
- 典拠: RS設計判断「判定を適用するbestmove」、PO「停止理由」。
- 前提: 通常探索、無限探索、または先読みが実行中で、深さ1以上の採用結果が閾値以下になる。
- 操作と期待観測: `stop`で停止し、`info string stop external`の後に`bestmove resign`をちょうど1行返す。
- 境界・不正: 深さ0ならD6-USI-50に従う。
- 性質: 実行中の停止にも同じ投了判定を適用する。

### D6-USI-55 完了保留中のstopによる投了
- 典拠: RS設計判断「判定を適用するbestmove」、PO「停止理由」。
- 前提: 先読みまたは無限探索の完了結果が閾値以下で保留されている。
- 操作と期待観測: `stop`前にはbestmoveを出さず、解放時に保持した停止理由と`bestmove resign`を1行ずつ返す。
- 境界・不正: 保留した完了深さが0なら投了しない。
- 性質: 保留の前後で評価値と完了深さを保つ。

### D6-USI-56 実行中のponderhit後の投了
- 典拠: RS設計判断「判定を適用するbestmove」、PO「的中」。
- 前提: 先読みが実行中である。
- 操作と期待観測: `ponderhit`では探索を継続し、その後の完了結果が深さ1以上かつ閾値以下なら停止理由と`bestmove resign`を返す。
- 境界・不正: hit前にbestmoveを出さず、hitだけで探索を停止しない。
- 性質: 通常完了と同じ採用結果で判定する。

### D6-USI-57 完了保留中のponderhitによる投了
- 典拠: RS設計判断「判定を適用するbestmove」、PO「的中」。
- 前提: 深さ1以上かつ閾値以下の先読み結果が保留されている。
- 操作と期待観測: `ponderhit`で保持した停止理由と`bestmove resign`を直ちに1行ずつ返す。
- 境界・不正: 保留した深さ0の結果は通常の着手を返す。
- 性質: 解放時に再探索しない。

### D6-USI-58 投了時の出力順と採用結果
- 典拠: RS設計判断「出力の形式」「判定の入力」、D6-USI-38。
- 前提: 投了条件を満たす採用結果が得られる。
- 操作と期待観測: 既存のinfo列に続いて`info string stop <理由>`、`bestmove resign`の順に出力する。予想手を付けず、投了のためのinfo再出力を行わない。
- 境界・不正: 最後の進捗報告と採用結果の評価値が異なる場合も採用結果で判定する。
- 性質: bestmoveは1行だけであり、その後にinfoを出さない。

### D6-USI-59 投了後の局面とライフサイクル
- 典拠: RS設計判断「局面の状態」、D6-USI-24、D6-USI-33。
- 前提: `position sfen`で同期した局面から投了する。
- 操作と期待観測: 投了前後の`state`と`moves`は同一であり、同じ局面から再探索できる。`position`による同期と`gameover`後の次局開始も従来どおりである。
- 境界・不正: 投了によってstatusをresignationへ変更せず、gameover後のstateは既定のエラーを返す。
- 性質: 投了はEngine.gameを変更しない。

### D6-USI-60 破棄した探索は投了を送出しない
- 典拠: RS設計判断「判定を適用するbestmove」、D6-USI-23、D6-USI-45。
- 前提: 実行中または完了保留中の探索が投了条件を満たす。
- 操作と期待観測: `gameover`、`quit`、または先読み中の入力切断で結果を破棄するとbestmoveを出さない。
- 境界・不正: 破棄後のstateや次局開始は既存契約に従う。
- 性質: 投了判定はbestmoveを解放する経路に限られる。
...
- 仕様化した挙動: 108件（USI 60、CECP 34、ENG 8、CLI 6）

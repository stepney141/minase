...
固定PSTへ2駒関係の補正を加えるFactorization Machine（以下FM）は、`train.model = "fm"` と [train_fm.py](../tools/train/pst/train_fm.py) で学習する。
...
`fm` では共通の学習項目に加えて、`rank`、`weight_decay`、`lambda_res`、`patience` の4項目をすべて指定する。
開始条件は潜在次元32、重み減衰0.0001、補正量の正則化係数0.001、忍耐回数3エポックとし、学習率0.001、最大20エポック、バッチサイズ4,096でAdamWを使う。
これら4項目は `single` と `tapered` では受理しない。
FMの第1フェーズでは `generate.seeds = []` として既存データだけを使う。
...
スクリプトは既存データと生成データを検証し、`single` と `tapered` では世代ごとの教師尺度と全体のモデル出力尺度Kを推定する。
`fm` では世代別の教師尺度だけを推定して世代番号順の `--teacher-ks` で渡し、出力尺度Kは基準MNPTの復号値に固定する。
...
`single` と `tapered` の重みファイルはMNPTバージョン2であり、序中盤用と終盤用の2組の重みに加え、基準から引き継いだ探索用駒価値47個を持つ。
...
PSTは1/8センチポーン単位へ量子化し、FMの埋め込みは量子化後の最大絶対値が16,383以下になる最大の2の累乗で量子化する。
いずれも検証標本で浮動小数点評価との平均絶対誤差が2センチポーン以下であることを確認する。
合格して正常終了した重みだけに `training/complete.json` が作られる。
`fm` は両端点PST、探索用駒価値、Kを基準から変えず、補正だけを加えたMNPTバージョン3を出力する。
量子化前の `training/pst-float.npz` には埋め込み `V`、出力係数 `a`、観測マスク `mask` を保存する。
特徴の観測回数、最初の5ステップの損失勾配ノルム、実際の更新回数、学習曲線、最良エポック、検証集合の補正量分布は `training/pst-training.json` に記録する。
最良エポックが0なら量子化せず、MNPTと `complete.json` を作らずに `excluded.json` を保存して正常終了する。
この学習は候補の選択と診断から除外する。

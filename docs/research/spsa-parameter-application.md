...
- 現行の`src/search/params.rs`は`cargo fmt --check`を通る。既定値を6桁へ変えた写しと、宣言の行の空白と字下げを崩した写しに`rustfmt --edition 2024`をかけても変化しなかった。
...
minaseの現行手引きは、最終値を四捨五入して`src/search/params.rs`の既定値へ書き込み、コミットしてから棋力を検定する手順を定めている。
...
以上から、minaseでは保存記録を検証して最終値を取り出し、`src/search/params.rs`の既定値だけを更新する独立した`apply`コマンドを追加する方針が考えられる。

監査範囲は `src/bin/**`、[tests/match_runner.rs](/home/stepney141/board-games/minase/tests/match_runner.rs)、`tools/train/**`、`scripts/**` である。
...
判断の根拠は [docs/sprt.md](/home/stepney141/board-games/minase/docs/sprt.md)、[docs/pst-training.md](/home/stepney141/board-games/minase/docs/pst-training.md)、[docs/plans/spec-first-tests/matrices/d8-stats-harness.md](/home/stepney141/board-games/minase/docs/plans/spec-first-tests/matrices/d8-stats-harness.md) と同ledgers/d8、[docs/plans/match-harness-efficiency.md](/home/stepney141/board-games/minase/docs/plans/match-harness-efficiency.md)、[docs/plans/random-play.md](/home/stepney141/board-games/minase/docs/plans/random-play.md)、[docs/plans/evaluation-gen1.md](/home/stepney141/board-games/minase/docs/plans/evaluation-gen1.md)、[docs/plans/tapered-pst.md](/home/stepney141/board-games/minase/docs/plans/tapered-pst.md)、教訓索引である。
...
対象は [src/bin/match_runner.rs:3533](/home/stepney141/board-games/minase/src/bin/match_runner.rs:3533) の `delayed_head_pair_does_not_leave_a_worker_idle` 全体である。
...
対象は [src/bin/match_runner.rs:4838](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4838) の `game_outcomes_map_to_candidate_pair_score_categories` 内、4885行以降の「2局とも勝ち」「1勝1敗」「2局とも反則負け」に対する `half_points(...) + half_points(...)` のassertである。
...
対象は [src/bin/bench.rs:388](/home/stepney141/board-games/minase/src/bin/bench.rs:388) の `total_node_counts_are_reproducible_across_runs` と364行の `bench_positions_are_the_fifteen_documented_valid_games` である。
...
対象は [tests/match_runner.rs:98](/home/stepney141/board-games/minase/tests/match_runner.rs:98) の `random_usi_match_is_reproducible_for_the_same_seed` と108行の `random_usi_match_output_is_independent_of_concurrency` である。
...
対象は [src/bin/selfplay_gen.rs:1197](/home/stepney141/board-games/minase/src/bin/selfplay_gen.rs:1197) の `play_game_is_deterministic_for_same_seed_and_arguments` と1273行の `records_start_at_or_after_planned_injection_boundary` である。
...
対象は [tools/train/pst/test_train_pst.py:506](/home/stepney141/board-games/minase/tools/train/pst/test_train_pst.py:506) の `TaperedFormatTest.test_make_model_rejects_unexpected_columns` 全体である。
...
対象は [tools/train/pst/test_train_pst.py:218](/home/stepney141/board-games/minase/tools/train/pst/test_train_pst.py:218) の `TrainingPathTest.test_best_epoch_replacement_requires_strict_improvement` 全体である。
...
対象は [src/bin/minase.rs:155](/home/stepney141/board-games/minase/src/bin/minase.rs:155) の `rules_argument_shares_the_wire_value_grammar`、[src/bin/random_play.rs:625](/home/stepney141/board-games/minase/src/bin/random_play.rs:625) の `rules_argument_resolves_presets_case_insensitively_and_rejects_combinations`、[src/bin/match_runner.rs:4499](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4499) の `rules_presets_resolve_per_article_33` である。
共通側の保持先は [src/core/rules.rs:2211](/home/stepney141/board-games/minase/src/core/rules.rs:2211) の `article_33_5_and_33_6_presets_expand_from_rule_constants`、2228行の `rule_set_parse_errors_preserve_the_failure_kind`、2243行の `rule_set_display_includes_all_four_base_groups` である。
...
対象は [src/bin/random_play.rs:683](/home/stepney141/board-games/minase/src/bin/random_play.rs:683) の `same_seed_and_game_number_reproduce_the_same_game` と698行の `verify_all_verification_completes_on_a_short_game` である。
...
対象は [src/bin/selfplay_gen.rs:1394](/home/stepney141/board-games/minase/src/bin/selfplay_gen.rs:1394) の `recorded_statistics_counts_every_repeated_search_key` と1476行の `completed_games_are_merged_independently_of_arrival_order` である。
...
対象は [tools/train/pst/test_train_pst.py:468](/home/stepney141/board-games/minase/tools/train/pst/test_train_pst.py:468) の `test_single_model_requires_identical_endpoints_and_duplicates_output` 内、491行の `for model in ("single", "tapered")` のsingle正常学習だけである。
singleの両端点一致と出力駒価値は、同ファイル223行の実学習テストと [test_pst_workflow.py:205](/home/stepney141/board-games/minase/tools/train/pst/test_pst_workflow.py:205) の `test_cpu_training_and_diagnostics_preserve_scale_and_outputs` が確認する。
...
対象は [src/bin/match_runner.rs:4380](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4380) の `clock_update_adds_the_increment_after_each_move` と4397行の `go_time_arguments_reflect_current_clocks_and_cross_colors` である。
...
対象は [src/bin/match_runner.rs:4242](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4242) の `hash_options_resolve_independently_for_each_engine` 内の4表行である。
...
対象は [src/bin/usi_random.rs:221](/home/stepney141/board-games/minase/src/bin/usi_random.rs:221) の `identical_seeds_reproduce_identical_outputs` 内、238〜252行のGame生成、`legal_moves`、同じ `XorShift64::index`、`usi::text` による期待値生成と最初のbestmove照合である。
本番 `handle_go`（143行）と同じAPI・同じ選択式を再実行しており、乱数indexや表記の共通バグは両側へ伝播する。内部合法手の列挙順を変えれば外部仕様を保つ変更でも期待値生成が結びついたままである。
同一入力の出力一致、goごと1応答はその場に残す。合法手を返す契約は実際の審判を通る [tests/match_runner.rs](/home/stepney141/board-games/minase/tests/match_runner.rs) のrandom対局が残す。
...
対象は [src/bin/selfplay_gen.rs:1306](/home/stepney141/board-games/minase/src/bin/selfplay_gen.rs:1306) の `injection_plan_has_unique_offsets_and_planned_boundary` 内の `[68,72,82]` とrecord_from=83の固定assert、および1333行の `zero_random_moves_produces_empty_plan_at_opening_boundary` の開始手数8/12/16の3行である。
...
対象は [src/bin/selfplay_gen.rs:1214](/home/stepney141/board-games/minase/src/bin/selfplay_gen.rs:1214) の `injection_counts_and_offsets_stay_within_configured_bounds` の対局番号1..=8である。
...
対象は [src/bin/match_runner.rs:4922](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4922) の `pair_seed_derivation_is_deterministic_nonzero_and_distinct` 内、100個を2回生成しsort/dedupする部分である。
実ペア配線が同じシードを再利用してしまう不具合は [tests/match_runner.rs:108](/home/stepney141/board-games/minase/tests/match_runner.rs:108) が出力された4個のpair_seedの相異で検査し、再現性は同じプロセス対局比較で検査する。
このテストに残すのは出力0の原像に対する仕様上の非零置換1件である。[src/rng.rs](/home/stepney141/board-games/minase/src/rng.rs) の `derive_seed` には専用テストがなく、random_playの `game_seed` は別実装なので、この境界まで重複として削除してはいけない。
100件の有限列挙は一般的な単射性の証明にならず、出力が常に同じになる代表的故障は統合側で残る。

T18では、開始局面を二重に作る再現性比較を統合テストへ任せる。

対象は [src/bin/match_runner.rs:4942](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4942) の `openings_stay_within_8_to_12_plies_and_derive_deterministically` 内、同じpair_seedでreplayを作って `moves` と `usi_moves` を比較する2assertと4回の再生成である。
...
対象は [tests/match_runner.rs:17](/home/stepney141/board-games/minase/tests/match_runner.rs:17) の `run_random_match` 冒頭にある `CARGO_BIN_EXE_match_runner` の隣のusi_randomパスと `CARGO_BIN_EXE_usi_random` を比較するassertである。
これはCargoの配置とテスト自身の `with_file_name` を比較し、本番の `resolve_player` を呼んでいない。実際のrunner起動が続くので、random解決の破損は後続の正常終了assertで検出できる。
また [src/bin/match_runner.rs:4008](/home/stepney141/board-games/minase/src/bin/match_runner.rs:4008) の `cecp_resolution_rejects_unsupported_search_limits` の全ケースに付いた `!error.to_string().is_empty()` は、`InvalidInput` の検査に対して仕様上の追加保証がないので削れる。
いずれも本番の現実的な検出力を失わない部分削除である。

T20では、外部形式の検査を残し、同じ値のserde往復を減らす。

対象は [src/bin/match_runner/storage.rs:958](/home/stepney141/board-games/minase/src/bin/match_runner/storage.rs:958) の `stop_reason_record_uses_fixed_snake_case_words` 内、5列挙値それぞれの `from_value(to_value(reason)) == reason` と、928行の `turn_record_round_trip_preserves_stop_data_and_nulls` のwith_values側往復である。
...
対象は [src/bin/match_runner/storage.rs:976](/home/stepney141/board-games/minase/src/bin/match_runner/storage.rs:976) の `resume_rejects_format_version_two` の `unsupported manifest format version 2` 完全文字列assert、[src/bin/match_report.rs:783](/home/stepney141/board-games/minase/src/bin/match_report.rs:783) の `report_rejects_format_version_two` の同様の全文assert、[src/bin/selfplay_gen.rs:1488](/home/stepney141/board-games/minase/src/bin/selfplay_gen.rs:1488) の `completed_game_merge_rejects_a_missing_game_number` の `worker channel closed after 1 of 4 games` 全文assertである。
...
また [src/bin/usi_random.rs:204](/home/stepney141/board-games/minase/src/bin/usi_random.rs:204) の `handshake_declares_ruleset_and_seed_defaults` はid authorの具体名や行順まで含む全stdout一致をやめ、必要なid行、option宣言、usiok/readyokを検査する。Seed既定の具体値と説明文はD8のSU-08/09が明文外と認めているので、これを独立仕様の保証とは数えない。
例外分類や必須wire tokenを消す提案ではなく、自由な文言と手順の固定だけを削る。

T22では、診断中の本番評価による期待値再計算を、独立な駒得へ置き換える。

対象は [tools/train/pst/test_pst_diagnostics.py:141](/home/stepney141/board-games/minase/tools/train/pst/test_pst_diagnostics.py:141) の `test_differing_endpoints_change_removal_deltas_with_the_phase` 内、149〜156行の `feature_indices`、`phase_numerators`、`integer_evaluate` による期待差の再計算である。
...
対象は [tools/train/pst/test_pst_workflow.py:205](/home/stepney141/board-games/minase/tools/train/pst/test_pst_workflow.py:205) の `test_cpu_training_and_diagnostics_preserve_scale_and_outputs` 末尾、`report["rust_agreement"] == {"base": validation, "candidate": validation}` のassertである。
...
[test_pst_diagnostics.py:75](/home/stepney141/board-games/minase/tools/train/pst/test_pst_diagnostics.py:75) の保存済み標本・件数の検査を1箇所に残し、同162行の `test_undefined_correlations_and_mismatched_probe_are_reported` のwrong_probe拒否も残す。後者は返値を故意にずらして比較配線の故障を検出するので、モックだから削る対象ではない。
...
対象は [tools/train/pst/test_train_pst.py:223](/home/stepney141/board-games/minase/tools/train/pst/test_train_pst.py:223) の `test_training_path_uses_documented_data_membership` 内、276〜312行の特徴抽出、`np.bincount`、未観測数・最大観測数、mirrorで同じカウントを作るfixture検査、および350行付近のログからその内部集計値を比較する部分である。
...
対象は [tools/train/pst/test_train_pst.py:68](/home/stepney141/board-games/minase/tools/train/pst/test_train_pst.py:68) の `reference_hash64`、93行の `test_hash_split_is_stable_when_file_order_changes` の80個の期待ハッシュ生成、および223行テストの `labels` である。
参照関数はmnsd.pyのハッシュを別の数値型でほぼ同じ手順として書き直しており、「指示書」とのコメントに反して、現行設計書はシード・対局番号のハッシュを20で割る分割規則までしか明文化していない。
一方、既存分割を保つ価値は高く、ここを根拠なく削除してはいけない。教訓 [validation-split-change-invalidates-old-baseline.md](/home/stepney141/board-games/minase/docs/lessons/validation-split-change-invalidates-old-baseline.md) が分割変更による比較破綻を明記している。
...
[src/bin/match_runner.rs:3429](/home/stepney141/board-games/minase/src/bin/match_runner.rs:3429) の `linux_resource_probe_reports_current_process_cpu_and_memory` は実環境依存で、特にphysical_core_count/physical_memoryが必ずSomeというassertは隔離環境で不安定になり得る。
...
残る [tools/train/pst/features.py](/home/stepney141/board-games/minase/tools/train/pst/features.py)、[mnsd.py](/home/stepney141/board-games/minase/tools/train/pst/mnsd.py)、[pst_diagnostics.py](/home/stepney141/board-games/minase/tools/train/pst/pst_diagnostics.py)、[pst_workflow.py](/home/stepney141/board-games/minase/tools/train/pst/pst_workflow.py)、[taper.py](/home/stepney141/board-games/minase/tools/train/pst/taper.py)、[taper_report.py](/home/stepney141/board-games/minase/tools/train/pst/taper_report.py)、[train_pst.py](/home/stepney141/board-games/minase/tools/train/pst/train_pst.py) と設定ファイルには別個のtest関数はない。
[scripts/clock_profile.py](/home/stepney141/board-games/minase/scripts/clock_profile.py)、[fetch_lishogi_replays.py](/home/stepney141/board-games/minase/scripts/fetch_lishogi_replays.py)、[hachu_replay.py](/home/stepney141/board-games/minase/scripts/hachu_replay.py)、[hachu_selfplay.py](/home/stepney141/board-games/minase/scripts/hachu_selfplay.py)、[match_cost_profile.py](/home/stepney141/board-games/minase/scripts/match_cost_profile.py) にもtest関数やunittest/pytest/doctestはない。

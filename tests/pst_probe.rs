...
fn flags_preserve_json_fields_and_side_relative_deltas() {
    ...
    // 学習済みFMの初期局面評価はPython参照値43cp、固定PSTは67cp。
    let candidate = probe(
        &path,
        concat!(env!("CARGO_MANIFEST_DIR"), "/nets/pst.bin"),
        &["--moves", "--promotions"],
    );
    assert_eq!(candidate[0]["eval"], 43);
    assert_eq!(candidate[0]["eval_pst"], 67);
    // Python参照で、両手番の歩兵捕獲はFM込み10cp、成り捕獲は305cp。
    for index in [1, 2] {
        assert_eq!(candidate[index]["eval"], 62);
        ...
        for row in candidate[index]["moves"].as_array().unwrap() {
            ...
            pawn_moves += 1;
            let (delta, delta_pst) = if mv.promote { (305, 268) } else { (10, -159) };
            assert_eq!(row["delta"], delta);
